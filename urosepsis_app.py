...
if st.button("🔍 Рассчитать риск уросепсиса", type="primary", use_container_width=True):
    ...
    # ---------- ДЕТАЛИ РАСЧЁТА ----------
    with st.expander("📐 Детали вычислений"):
        st.markdown(
            f"""
            **Формула логистической регрессии:**

            z = −15.928 + 0.550 × t°C + 0.017 × СРБ + 0.995 × Лейкоцитоз − 0.202 × Ht

            **Подставленные значения:**
            - Температура: **{temp:.1f}°C**
            - СРБ: **{crp:.1f} мг/л**
            - Лейкоцитоз > 10×10⁹/л: **{'Да' if leukocytosis_val else 'Нет'}**
            - Гематокрит: **{ht:.1f}%**

            **Расчёт:**
            ```
            z = −15.928 + 0.550×{temp:.1f} + 0.017×{crp:.1f} + 0.995×{leukocytosis_val} − 0.202×{ht:.1f}
            z = {result['z']}
            P = 1 / (1 + e⁻ᶻ) × 100% = {result['probability']}%
            ```
            """
        )
    ...
    with st.expander("📊 Характеристики прогностической модели"):
        st.markdown(
            """
            | Параметр | Значение |
            |---|---|
            | AUC | 0.887 (95% ДИ: 0.821–0.953) |
            | R² Найджелкерка | 55.9% |
            | Чувствительность | 88.0% |
            | Специфичность | 84.2% |
            | Порог отсечения (индекс Юдена) | 57.2% |
            | Число наблюдений | 107 |
            | Метод | Бинарная логистическая регрессия |

            **Предикторы модели:**

            | Предиктор | AOR | 95% ДИ | p |
            |---|---|---|---|
            | СРБ (на 1 мг/л) | 1.017 | 1.007–1.027 | 0.001 |
            | Гематокрит (на 1%) | 0.817 | 0.706–0.951 | 0.009 |
            | Температура (на 1°C) | 1.733 | 0.934–3.217 | 0.082 |
            | Лейкоцитоз > 10×10⁹/л | 2.703 | 0.682–10.753 | 0.157 |

            *Источник: диссертация «Особенности беременности и родов при гестационном уросепсисе», РУДН, 2026.*
            """
        )
...
st.divider()
st.caption(
    "Диссертационное исследование • Кафедра акушерства и гинекологии с курсом перинатологии "
    "Медицинского института ФГАОУ ВО «РУДН им. Патриса Лумумбы» • Москва, 2026"
)
